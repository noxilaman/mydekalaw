	...
	public function createField($field,$operator,$data) {
		...
		if (strpos($field, '?') >= 0) {
			...
			foreach ($operator as $f) {
				...
				$escapeField = '';
				if (strpos(strtolower($field), 'like') !== false) {
					list($escapeField, $escapeValue) = $this->escapeLike($fdata[$f]);
					array_push($val, $escapeValue);
				} else {
					array_push($val, $fdata[$f]);
				}
			}

			if (!empty($escapeField)) {
				$field = str_replace(array('like','LIKE','?'), '', $field);
				$out[$field . $escapeField] = $val;
			} else {
				$out[$field] = $val;
			}
